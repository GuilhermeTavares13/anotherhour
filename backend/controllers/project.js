...
exports.getProject = async (req, res, next) => {
    ...
    if(!user) {
        res.status(404).json({ message: 'User not found!', project: {} });    
        return;
    } 
  
    const userProject =  await user.getProjects({ where: { id: projectId } });    
    
    if ((!userProject) || (!userProject.length)) {
        res.status(404).json({ message: 'Failed to fetch user project', project: {}});    
        return;
    }

    res.status(200).json({ message: 'Project fetched successfully', project: userProject});
    return;
}
